...
namespace Abbey.PdfProcessor.RealStatementProcessor.Parsers {
    internal class RelianceStandardPdfParser : PdfParser {
        /// <summary>
        ...
        public static ParseOutput ParsePdf ( string pdfFilePath ) {
            try {
                return ParsePdf( pdfFilePath, "RelianceStandard.ConverterX" );
            }
            catch (Exception e) {
                var errorMessage = new StringBuilder();
                errorMessage.AppendFormat( "Error Parsing Reliance Standard PDF: {0}", pdfFilePath );
