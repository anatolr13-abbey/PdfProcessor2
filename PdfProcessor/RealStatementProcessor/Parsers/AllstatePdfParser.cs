...
namespace Abbey.PdfProcessor.RealStatementProcessor.Parsers {
    internal class AllstatePdfParser : PdfParser {
        /// <summary>
        ...
        public static ParseOutput ParsePdf ( string pdfFilePath ) {
            try {
                return ParsePdf( pdfFilePath, "Allstate.ConverterX" );
            }
            catch (Exception e) {
                var errorMessage = new StringBuilder();
                errorMessage.AppendFormat( "Error Parsing Allstate PDF: {0}", pdfFilePath );
