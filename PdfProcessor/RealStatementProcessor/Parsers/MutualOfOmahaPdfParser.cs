...
namespace Abbey.PdfProcessor.RealStatementProcessor.Parsers {
    internal class MutualOfOmahaPdfParser : PdfParser {
        /// <summary>
        ...
        public static ParseOutput ParsePdf ( string pdfFilePath ) {
            try {
                return ParsePdf( pdfFilePath, "MutualOfOmaha.ConverterX" );
            }
            catch (Exception e) {
                var errorMessage = new StringBuilder();
                errorMessage.AppendFormat( "Error Parsing Mutual Of Omaha PDF: {0}", pdfFilePath );
