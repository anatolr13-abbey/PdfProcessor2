...
namespace Abbey.PdfProcessor.RealStatementProcessor.Parsers {
    internal class MassMutualPdfParser : PdfParser {
        /// <summary>
        ...
        public static ParseOutput ParsePdf ( string pdfFilePath ) {
            try {
                return ParsePdf( pdfFilePath, "MassMutual.ConverterX" );
            }
            catch (Exception e) {
                var errorMessage = new StringBuilder();
                errorMessage.AppendFormat( "Error Parsing MassMutual PDF: {0}", pdfFilePath );
