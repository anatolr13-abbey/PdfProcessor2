...
namespace Abbey.PdfProcessor.RealStatementProcessor.Parsers {
    internal class EquitablePdfParser : PdfParser {
        /// <summary>
        ...
        public static ParseOutput ParsePdf ( string pdfFilePath ) {
            try {
                return ParsePdf( pdfFilePath, "Equitable.ConverterX" );
            }
            catch (Exception e) {
                var errorMessage = new StringBuilder();
                errorMessage.AppendFormat( "Error Parsing Equitable PDF: {0}", pdfFilePath );
