...
namespace Abbey.PdfProcessor.RealStatementProcessor.Parsers {
    internal class AssurityPdfParser : PdfParser {
        /// <summary>
        ...
        public static ParseOutput ParsePdf ( string pdfFilePath ) {
            try {
                return ParsePdf( pdfFilePath, "Assurity.ConverterX" );
            }
            catch (Exception e) {
                var errorMessage = new StringBuilder();
                errorMessage.AppendFormat( "Error Parsing Assurity PDF: {0}", pdfFilePath );
