...
namespace Abbey.PdfProcessor.RealStatementProcessor.Parsers {
    internal class BrightHousePdfParser : PdfParser {
        /// <summary>
        ...
        public static ParseOutput ParsePdf ( string pdfFilePath ) {
            try {
                return ParsePdf( pdfFilePath, "BrightHouse.ConverterX" );
            }
            catch (Exception e) {
                var errorMessage = new StringBuilder();
                errorMessage.AppendFormat( "Error Parsing BrightHouse PDF: {0}", pdfFilePath );
