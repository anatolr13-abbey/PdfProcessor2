...
namespace Abbey.PdfProcessor.RealStatementProcessor.Parsers {
    internal class SecurityMutualPdfParser : PdfParser {
        /// <summary>
        ...
        public static ParseOutput ParsePdf ( string pdfFilePath ) {
            try {
                return ParsePdf( pdfFilePath, "SecurityMutual.ConverterX" );
            }
            catch (Exception e) {
                var errorMessage = new StringBuilder();
                errorMessage.AppendFormat( "Error Parsing Security Mutual PDF: {0}", pdfFilePath );
